...
class AboutTrueAndFalse(Koan):
    ...
    def test_true_is_treated_as_true(self):
        self.assertEqual('true stuff', self.truth_value(True)) # if condition is shorthand for if condition == True.

    def test_false_is_treated_as_false(self):
        self.assertEqual('false stuff', self.truth_value(False)) # otherwise it's false

    def test_none_is_treated_as_false(self):
        self.assertEqual('false stuff', self.truth_value(None)) # None is falsy

    def test_zero_is_treated_as_false(self):
        self.assertEqual('false stuff', self.truth_value(0)) # 0 is falsy
    ...
    def test_blank_strings_are_treated_as_false(self):
        self.assertEqual('false stuff', self.truth_value("")) # empty strings are falsy

    def test_everything_else_is_treated_as_true(self):
        self.assertEqual('true stuff', self.truth_value(1))
        self.assertEqual('true stuff', self.truth_value([0])) # lists etc. have content even if the content is just the number 0 so truthy
        self.assertEqual('true stuff', self.truth_value((0,)))
        self.assertEqual(
            'true stuff',
            self.truth_value("Python is named after Monty Python")) # A full string is truthy
